...
class AccessService {
    ...
    static signUp = async ({ email, password, roleId = 'R3' }) => {
        if (!email || !password)
            throw new BadRequestError(`Invalid input email/ password`)
        //check exist email 
        ...
        const newUser = await db.User.create({
            email, password: passwordHash, roleId
        });
        // console.log(newUser)
        if (newUser) {
    ...
    static signIn = async ({ email, password }) => {
        ...
        const tokens = await createTokenPair({
            role: holderEmail.roleId, email
        },
            holderKey.public_key,
            holderKey.private_key);
        console.log(`Created tokens success:: `, tokens)
        // console.log(`created token`)
        const addRefreshToken = await UserService.updateRefreshToken(tokens.refreshToken, holderEmail.id);
        ...
        return {
            user: getInfoData({ fields: ['roleId', 'email', 'image', 'firstName', 'lastName'], object: holderEmail }),
            token: tokens
        }

        // //succeed login
        // return _.merge(
        //     getInfoData({ fields: ['roleId', 'email', 'image'], object: holderEmail }),
